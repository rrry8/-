...
//カメラ回転時の感度調整
constexpr float SENSITIVITY_DIV = 30.0f;
...
void Camera::Update(bool dashFlag)
{
	if (GetJoypadNum())
	{
		...
		//マウス位置の差分で回転させる
		angle_.y += Deg2RadF(padMoveDistance.x / SENSITIVITY_DIV);	//左右回転
		angle_.x += Deg2RadF(padMoveDistance.y / SENSITIVITY_DIV);	//上下回転
		//上方向の角度制限
...
void Camera::UpdateRotateForMouse(bool dashFlag)
{
	Vector2Int centerMousePos = { lpSceneMng.GetScreenSize().x / 2, lpSceneMng.GetScreenSize().y / 2 };
	//マウスの位置を取得
	GetMousePoint(&mousePos_.x, &mousePos_.y);

	//マウスの移動距離を計算
	Vector2 mouseMoveDistance = { static_cast<float>(mousePos_.x - centerMousePos.x),static_cast<float>(mousePos_.y - centerMousePos.y) };

	//マウス位置の差分で回転させる
	angle_.y += Deg2RadF(mouseMoveDistance.x / SENSITIVITY_DIV);	//左右回転
	angle_.x += Deg2RadF(mouseMoveDistance.y / SENSITIVITY_DIV);	//上下回転
	...
	//マウスの位置を真ん中に固定
	SetMousePoint(centerMousePos.x, centerMousePos.y);
}
