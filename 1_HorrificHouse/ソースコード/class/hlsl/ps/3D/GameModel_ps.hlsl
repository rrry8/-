...
PS_OUTPUT main(PS_INPUT input)
{
    ...
    //0.0f~1.0fに変換
    //範囲外は1か0
    if (input.depthView.z < depthRange.x)
    {
        output.depth.r = 0.0f;
    }
    else if (input.depthView.z > depthRange.y)
    {
        output.depth.r = 1.0f;
    }
    else
    {
        output.depth.r = (input.depthView.z - depthRange.x) * depthRange.z;
    }
