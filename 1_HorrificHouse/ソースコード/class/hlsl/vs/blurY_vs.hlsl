...
VS_OUTPUT main(VS_INPUT input)
{
	...
	float2 textureSize = float2(1920.0f, 640.0f);

	//基準のUV座標
	float2 textureUV = input.uv0;

	//　テクセルずらす
	output.uv0.xy = float2(0.0f, 1.0f / textureSize.y);
	output.uv1.xy = float2(0.0f, 3.0f / textureSize.y);
	output.uv2.xy = float2(0.0f, 5.0f / textureSize.y);
	output.uv3.xy = float2(0.0f, 7.0f / textureSize.y);
	output.uv4.xy = float2(0.0f, 9.0f / textureSize.y);
	output.uv5.xy = float2(0.0f, 11.0f / textureSize.y);
	output.uv6.xy = float2(0.0f, 13.0f / textureSize.y);
	output.uv7.xy = float2(0.0f, 15.0f / textureSize.y);
