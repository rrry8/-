...
void InventoryScene::Init(void)
{
    ...
    CalcBlurWeights(weights);

    //定数バッファ初期化、初期化時に一度更新
    for (int n = 0; n < WEIGHTS_NUM; n++)
    {
        lpCBufferMng.SetPixelCbuffer(9, n, weights[n]);
    }
    lpCBufferMng.Update();
...
void InventoryScene::CalcBlurWeights(float* weights)
{
    //重みの合計
...
void InventoryScene::DrawBackGround(void)
{
    //メインレンダリングターゲットに描画
...
void InventoryScene::InitTable(void)
{
    ...
    explanationTable_ = {
        {ItemID::Entrance_Key,"Entrancekeydescribe"},
        {ItemID::Kitchen_Key,"kitchenkeydescribe"},

        {ItemID::MusicRoom_Key,"musicroomkeydescribe"},
        {ItemID::FoodBank_Key,"Pantrykeydescribe"},
        {ItemID::ChildRoom_Key,"childRoomkeydescribe"},
        {ItemID::Celler_Key,"basementkeydescribe"},
        {ItemID::Phot,"Photdescide"},
        {ItemID::Story_Book_A,"storymemo_a"},
        {ItemID::Story_Memo_B,"storymemo_b"},
        {ItemID::Story_Book_C,"storymemo_c"},
        {ItemID::Enemy_Memo,"enemymemo"},
        {ItemID::Wall_Memo,"hint_d"},
        {ItemID::Picture_Memo,"hint_q"},
        {ItemID::CookWare_Memo,"hint_f"},
        {ItemID::Score_Memo,"hint_g"},
        {ItemID::Living_Memo,"hint_h"},
        {ItemID::Clock_Book,"hint_j"},
        {ItemID::Celler_Memo,"hint_t"},
        {ItemID::Mapitem,"mapItemdescide"}
