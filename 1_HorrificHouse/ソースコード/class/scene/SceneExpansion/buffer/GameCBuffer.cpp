...
//ライトバッファ
constexpr int CBUFFER_BLUR_AND_DEPTH = 9;
...
constexpr float POINT_RANGE0 = 1500;
...
//ブラー関係
constexpr int WEIGHTS_NUM = 8;		//重み数
constexpr float BLUR_SIGMA = 20.0f;	//ぼかしの度合い
...
void GameCBuffer::Init()
{
	...
	float totalWeight = 0.0f;	//重みの合計
	//xは基準テクセルからの距離
	for (int x = 0; x < WEIGHTS_NUM; x++)
	{
		weights[x] = expf(-0.5f * static_cast<float>(x * x) / BLUR_SIGMA);
		totalWeight += 2.0f * weights[x];
	}
	//除算し、合計を1にする
	...
	{
		lpCBufferMng.SetPixelCbuffer(CBUFFER_BLUR_AND_DEPTH, n, weights[n]);
	}
...
void GameCBuffer::Update(Vector3 cameraPos,Vector3 cameraTargetPos, Vector3 enemyEyePos, Vector3 enemyDir, Vector3 enemyTargetPos,Vector3 depthRange,Vector2 depthPos)
{
	...
	//被写界深度の範囲
	lpCBufferMng.SetPixelCbuffer(CBUFFER_BLUR_AND_DEPTH, WEIGHTS_NUM, depthRange.x);
	lpCBufferMng.SetPixelCbuffer(CBUFFER_BLUR_AND_DEPTH, WEIGHTS_NUM + 1, depthRange.y);
	lpCBufferMng.SetPixelCbuffer(CBUFFER_BLUR_AND_DEPTH, WEIGHTS_NUM + 2, depthRange.z);
	lpCBufferMng.SetPixelCbuffer(CBUFFER_BLUR_AND_DEPTH, WEIGHTS_NUM + 3, 0.0f);

	//フォーカス位置
	lpCBufferMng.SetPixelCbuffer(CBUFFER_BLUR_AND_DEPTH, WEIGHTS_NUM + 4, depthPos.x);
	lpCBufferMng.SetPixelCbuffer(CBUFFER_BLUR_AND_DEPTH, WEIGHTS_NUM + 5, depthPos.y);
