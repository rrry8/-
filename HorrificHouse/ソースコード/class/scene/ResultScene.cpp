...
//初期位置
constexpr VECTOR CAMERA_INIT_POS = { 0.0f, 400.0f, -400.0f };           //カメラ
constexpr VECTOR CLIFF_INIT_POS = { -470.0f, -440.0f, -2270.0f };       //崖
constexpr VECTOR WATER_INIT_POS = { 0.0f, 200.0f, 0.0f };               //水
constexpr VECTOR TAREGET_INIT_POS = { 0.0f, 400.0f, 400.0f };           //注視点
constexpr VECTOR GRASS_INIT_POS = { 8.0f, 175.0f, 0.0f };               //草
constexpr float ADD_ROTATE = 0.0001f;                                   //回転加算値

//キューブテクスチャ
constexpr int CUBE_TEXTURE_NUM = 6; //数
constexpr int CUBE_TEXTURE_SIZE = 512;      //テクスチャサイズ

constexpr float SKYDOME_SCALE = 30.0f;  //スカイドームの大きさ
constexpr float GRASS_SCALE = 0.4f;     //草の大きさ
...
void ResultScene::Init(void)
{
    ...
    cubeTexture_ = MakeScreen(CUBE_TEXTURE_SIZE, CUBE_TEXTURE_SIZE, true);
...
{
    if (resultUI_->GetHitUICol() == ResultColUI::reTitle)
    {
        //タイトルへ
