...
//制作者の位置
constexpr int CREATOR_POS_X = 960;  //横
constexpr int CREATOR_POS_Y = 300;  //縦

//役割の位置
constexpr int ROLE_POS_X = 960;     //横
constexpr int ROLE_POS_Y = 0;       //縦

//役割のオフセット
constexpr int ROLE_OFFSET_X = 250;  //横
constexpr int ROLE_OFFSET_Y = 50;   //縦
...
void ResultUI::Update()
{
    ...
    {
        UpdateCredit(creator_, { CREATOR_POS_X,CREATOR_POS_Y });
    }
    else
    {
        stepTime_ += lpSceneMng.GetDeltaTime();
    }
    UpdateCredit(role_, { ROLE_POS_X,ROLE_POS_Y });
}
...
void ResultUI::UpdateCredit(CreditData& creditData, Vector2Int pos)
{
    //停止時間内なら返す
